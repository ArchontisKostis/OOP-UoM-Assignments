...
public class Suspect {
	...
	private ArrayList<Suspect> possiblePartners, suggestedPartners;
	...
	public Suspect(String aName, String aUsername, String aCity) {
		...
		this.suggestedPartners = new ArrayList<Suspect>();
	}
	...
	public ArrayList<Suspect> findSuggestedSuspects() {
		ArrayList<Suspect> possibleSuspects = new ArrayList<Suspect>();
		
		/* For every partner find all his partners 
		 * that are not connected to current suspect 
		 * and add them to possibleSuspectsList
		 */
		for(Suspect partner: this.possiblePartners) {
			ArrayList<Suspect> partnerPartnersList = partner.getPartnersList();
			
			for(Suspect partnersPartner: partnerPartnersList) {
				if(!this.isConnectedTo(partnersPartner) && !this.userName.equals(partnersPartner.userName))
					possibleSuspects.add(partnersPartner);
			}
		}
